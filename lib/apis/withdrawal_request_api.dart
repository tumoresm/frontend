...
abstract class IWithdrawalRequestAPI {
  FutureEither<model.Document> createWithdrawalRequest(
      WithdrawalRequestModel request);
  FutureEither<model.Document> updateWithdrawalRequest(
      WithdrawalRequestModel request);
  Future<List<WithdrawalRequestModel>> getWithdrawalRequestsByUserId(
      String userId);
  Future<List<WithdrawalRequestModel>> getWithdrawalRequestsByStatus(
...
class WithdrawalRequestAPI implements IWithdrawalRequestAPI {
  ...
  FutureEither<model.Document> createWithdrawalRequest(
    ...
    if (_db == null) {
      Loggers.database
          .warning('Withdrawal request API not available during migration');
      return left(Failure(
          'Withdrawal request API not available during migration',
          StackTrace.current));
    }
    try {
  ...
  Future<List<WithdrawalRequestModel>> getWithdrawalRequestsByUserId(
    ...
    if (_db == null) {
      Loggers.database.warning(
          'Withdrawal request API not available - returning empty list');
      return [];
    }
    try {
  ...
  FutureEither<model.Document> cancelWithdrawalRequest(String requestId) async {
    ...
    try {
      ...
      if (!currentRequest.canBeCancelled) {
        return left(Failure(
            'This withdrawal request cannot be cancelled', StackTrace.current));
      }
